...
func run(cmd *cobra.Command, _ []string) error {
	...
	if adherenceErr != nil {
		...
		case apimeta.IsNoMatchError(adherenceErr):
			log.Info("TLS adherence API not available (non-OpenShift or pre-4.22 cluster)")
		case apierrors.IsNotFound(adherenceErr):
			log.Info("APIServer resource not found for adherence, skipping")
		case apierrors.IsServiceUnavailable(adherenceErr),
			...
			log.Info("Transient error fetching TLS adherence policy, watcher will retry", "error", adherenceErr)
		default:
			return fmt.Errorf("unable to read TLS adherence policy: %w", adherenceErr)
		}
	} else {
